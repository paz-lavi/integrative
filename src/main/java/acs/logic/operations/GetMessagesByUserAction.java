...
public class GetMessagesByUserAction implements ActionHandler {

	// this class must have a default constructor to support implementation based on java reflection
	public GetMessagesByUserAction() {
		System.err.println("**** GetMessagesByUserAction()");
	}
	
	@Transactional
	@Override
	public Object handleAction(ActionEntity action, ElementDao elementDao, MessageDao messageDao) {
		List<MessageEntity> messagges = messageDao.findAllByInvokedBy((UserId) action.getActionAttributes().get("user masseges"),PageRequest.of((int)action.getActionAttributes().get("page"),
